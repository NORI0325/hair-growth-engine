...
BEGIN
  ...
  SELECT
    ...
    u.email::text,
    COALESCE(
      (SELECT array_agg(lm.location_id) FROM public.location_members lm WHERE lm.user_id = tm.user_id),
      ARRAY[]::uuid[]
    ) AS location_ids,
    COALESCE(
...
REVOKE ALL ON FUNCTION public.get_tenant_members_detail(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_tenant_members_detail(uuid) TO authenticated;
