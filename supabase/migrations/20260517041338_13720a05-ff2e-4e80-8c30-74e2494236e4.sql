-- Phase 2: public_create_booking_v5 — sync_required判定と安全昇格
-- 外部連携liveがある店舗: status='pending'のまま (sync成功でcallbackがconfirmedに昇格)
-- 外部連携が一切ない店舗: status='confirmed', sync_status='not_required'
...
BEGIN
  ...
  -- Phase2: 必ず status='pending' で挿入。後でsync状況により昇格判定
  ...
  -- Phase2: sync不要なら即confirmed昇格 / 必要ならpendingのまま
