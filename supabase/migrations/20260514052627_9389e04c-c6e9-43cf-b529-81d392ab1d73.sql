...
-- 1. customers にカナ列追加
ALTER TABLE public.customers ADD COLUMN IF NOT EXISTS name_kana text;

-- 2. public_create_booking_v4: v3 とほぼ同じだが _full_name_kana を追加し
--    customers.name_kana に保存、sync_jobs.payload.customer_kana にも展開
CREATE OR REPLACE FUNCTION public.public_create_booking_v4(
  _salon_slug text,
  _full_name text,
...
BEGIN
  ...
  END IF;
  -- カタカナ・ひらがな・スペース・長音のみ許可（ひらがなはクライアント側で変換推奨だが許容）
  ...
  IF _staff_id IS NOT NULL THEN
    ...
    END IF;
  ELSE
    SELECT COUNT(DISTINCT s.id)::INTEGER INTO _slot_staff_count
    FROM public.staff s
    JOIN public.staff_schedules ss
      ON ss.staff_id = s.id AND ss.weekday = _weekday AND ss.active = true
    ...
    IF COALESCE(_slot_staff_count, 0) = 0 THEN
...
REVOKE ALL ON FUNCTION public.public_create_booking_v4(text, text, text, text, text, date, time, text[], text, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.public_create_booking_v4(text, text, text, text, text, date, time, text[], text, uuid) TO anon, authenticated;
