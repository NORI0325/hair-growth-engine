...
const BookingReminderEmail = ({ customerName, salonName = 'サロン', bookingDate, bookingTime, menu, bookingLink }: Props) => (
  <Html lang="ja">
    <Head />
    <Preview>明日のご予約のご案内</Preview>
    <Body style={main}>
      <Container style={container}>
        <Text style={eyebrow}>— RESERVATION REMINDER —</Text>
        ...
        <Text style={text}>
          ご予約の変更・キャンセルがございましたら、お早めにご連絡ください。
        </Text>
        {bookingLink && (
          <Section style={{ textAlign: 'center', margin: '32px 0' }}>
            <Button href={bookingLink} style={button}>予約を確認・変更する</Button>
          </Section>
        )}
        <Hr style={hr} />
...
export const template = {
  ...
  subject: (d: Record<string, any>) => `【明日のご予約】${d.salonName || 'サロン'}より`,
  displayName: 'リマインダー（前日）',
